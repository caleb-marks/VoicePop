...
enum SetupAssistant {
    ...
    private static var bundledClean: String { bundleURL.appendingPathComponent("Contents/MacOS/voxtype-clean").path }
    ...
    /// relaunching itself from /Applications.
    ...
    private static func needsSetup() -> Bool {
        if !isParakeetCapable(VoxtypeModel.bin) { return FileManager.default.isExecutableFile(atPath: bundledVoxtype) }
    ...
    private static func perform(report: @escaping (String, Double?) -> Void) throws {
        ...
        if !isParakeetCapable(VoxtypeModel.bin) {
            ...
            clearQuarantine(bundledVoxtype)
            _ = try run(bundledVoxtype, ["setup", "app-bundle"])
            clearQuarantine(voxtypeApp)
        ...
        if !fm.fileExists(atPath: configPath) {
            ...
            text = text.replacingOccurrences(of: "__VOICEPOP_ROOT__/bin/voxtype-clean", with: bundledClean)
        ...
        if let script = Bundle.main.url(forResource: "restart-voxtype", withExtension: "sh")?.path {
            ...
            NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: voxtypeApp), configuration: NSWorkspace.OpenConfiguration())
        }
    }
    ...
    private static func downloadModel(report: @escaping (String, Double?) -> Void) throws {
        ...
        task.arguments = ["setup", "--download", "--model", modelName, "--progress-format", "json", "--quiet"]
        let out = Pipe()
        let err = Pipe()
        ...
        out.fileHandleForReading.readabilityHandler = { handle in
            for line in buffer.append(handle.availableData) {
                guard let obj = try? JSONSerialization.jsonObject(with: Data(line)) as? [String: Any],
                      obj["event"] as? String == "progress",
                      let pct = obj["pct"] as? Double else { continue }
        ...
        guard task.terminationStatus == 0 else {
            let detail = stderrBuffer.text.trimmingCharacters(in: .whitespacesAndNewlines)
